var url = "google-covid-mobility/2020_id_region_mobility_report_edit.csv"
...
d3.csv(url).then(function(data) {
    ...
    var reg = d3.map(data, function(d) {return(d.sub_region)}).keys()
    console.log(reg)

    var allGroup = d3.map(data, function(d){return(d.code)}).keys()
    console.log(allGroup)

    var regionMap = reg.map( function(x, i){
        return {"region": x, "code": allGroup[i]}        
    }.bind(this));
    ...
    var selectOpt = d3.select(".select1")
        .selectAll("option")
        .data(regionMap)
        .enter()
        .append("option")
    ...
    line_transit.raise()
        ...
        function mousemove() {
            ...
            cases1
                .transition()
                .duration(1000)
                .text(d.hari+", "+formatTime2(d.date))
        
            cases2
                .transition()
                .text(locale.format(",")(d.kasus))
