...
d3.csv(url).then(function(data) {
    ...
    console.log(data)
        ...
        var xMax = d3.max(data, d => parseInt(d.gdpkapita))
        console.log(xMax)
        ...
        var x = d3 //x - scale
            .scaleLinear()
            .domain(d3.extent(data, d=> +d.gdpkapita))
            .range([100,w-20]);
        ...
        .attr("x", d => x(d.total))
        .attr("y", d => y(d.negara))
        .attr("width", function(d) { return Math.abs(x(d.gdpkapita) - x(d.total)); })
        .attr("height",y.bandwidth())
        .on("mouseover",function(d) {            
            tooltip
            .html("<strong>" + d.negara + "</strong>" + "<br>USD " + d3.format(",.2r")(d.gdpkapita))
            //
            d3
            .select(this)
            .style("stroke","black")
        ...
        .style("fill","black")
        .attr("x", d => x(d.gdpkapita)+5)
        .attr("y", d => y(d.negara)+10)
        .text(function(d) { return d3.format(",.2r")(d.gdpkapita)})  
